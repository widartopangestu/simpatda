...
class GolonganController extends Controller {
    ...
    public function actionCreate() {
        ...
        if (isset($_POST['Golongan'])) {
            ...
            if ($model->save()) {
                Yii::app()->util->setLog(AccessLog::TYPE_SUCCESS, Yii::t('trans', 'Create Golongan ID : ') . $model->primaryKey);
    ...
    public function actionUpdate($id) {
        ...
        if (isset($_POST['Golongan'])) {
            $model->attributes = $_POST['Golongan'];
            if ($model->save()) {
                Yii::app()->util->setLog(AccessLog::TYPE_SUCCESS, Yii::t('trans', 'Update Golongan ID : ') . $model->id);
                $this->redirect(array('index'));
            }
        }
        ...
        if (Yii::app()->request->isPostRequest) {
            ...
            try {
                if ($this->loadModel($id)->delete())
                    Yii::app()->util->setLog(AccessLog::TYPE_SUCCESS, Yii::t('trans', 'Delete Golongan ID : ') . $id);
            } catch (CDbException $exc) {
                throw new CHttpException(500, Yii::t('trans', 'Delete Golongan ID : {id}. Item ini sudah dipakai pada transaksi', array('{id}' => $id)));
            }
    ...
    public function actionIndex() {
        ...
        Yii::app()->util->setLog(AccessLog::TYPE_INFO, Yii::t('trans', 'Manage Golongan'));
