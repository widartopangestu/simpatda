...
class JabatanController extends Controller {
    ...
    public function actionCreate() {
        ...
        if (isset($_POST['Jabatan'])) {
            ...
            if ($model->save()) {
                Yii::app()->util->setLog(AccessLog::TYPE_SUCCESS, Yii::t('trans', 'Create Jabatan ID : ') . $model->primaryKey);
    ...
    public function actionUpdate($id) {
        ...
        if (isset($_POST['Jabatan'])) {
            $model->attributes = $_POST['Jabatan'];
            if ($model->save()) {
                Yii::app()->util->setLog(AccessLog::TYPE_SUCCESS, Yii::t('trans', 'Update Jabatan ID : ') . $model->id);
                $this->redirect(array('index'));
            }
        }
        ...
        if (Yii::app()->request->isPostRequest) {
            ...
            try {
                if ($this->loadModel($id)->delete())
                    Yii::app()->util->setLog(AccessLog::TYPE_SUCCESS, Yii::t('trans', 'Delete Jabatan ID : ') . $id);
            } catch (CDbException $exc) {
                throw new CHttpException(500, Yii::t('trans', 'Delete Jabatan ID : {id}. Item ini sudah dipakai pada transaksi', array('{id}' => $id)));
            }
    ...
    public function actionIndex() {
        ...
        Yii::app()->util->setLog(AccessLog::TYPE_INFO, Yii::t('trans', 'Manage Jabatan'));
