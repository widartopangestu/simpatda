...
class MPTranslate extends CApplicationComponent {
    ...
    function editLink($label = 'Edit translations', $type = 'link') {
        $url = Yii::app()->getController()->createUrl('/translate/edit/index');
        if ($type === 'button')
    ...
    function getLanguage() {
        ...
        if (($language = @$this->_cache['language']) !== null)
            return $language;
        elseif (isset(Yii::app()->params['language']))
    ...
    function dropdown() {
        ...
        ');
        return CHtml::dropDownList(self::ID, $this->getLanguage(), $this->acceptedLanguages, array('id' => self::ID)
        );
    }
