...
$this->modulTitle = Yii::t('trans', 'Create') . ' ' . Yii::t('trans', 'SPTPD Pajak Walet');
$this->menu = array(
    array('label' => Yii::t('trans', 'Manage'), 'url' => array('index', 'jenis' => Spt::JENIS_PAJAK_WALET), 'icon' => 'list-alt', 'visible' => (Yii::app()->util->is_authorized('spt.index')) ? true : false),
);
?>
<div class="form">
    ...
    <div class="row">
        ...
        <div class="span5">
            ...
            <?php // echo $form->dropdownListControlGroup($model, 'kode_rekening_id', KodeRekening::model()->getParentTreeOptions(Spt::PARENT_WALET), array('span' => 3, 'empty' => Yii::t('trans', '-- Pilih Kode Rekening --'), 'onchange' => 'getNamaRekening($(this).val())')); ?>
        ...
        <div class="span-12">
            <fieldset><legend><?php echo Yii::t('trans', 'Detail Walet'); ?></legend>
                <table class="table table-striped table-bordered items-list">
                    <thead>
                        <tr>
                            <th><?php echo Yii::t('trans', 'Kode Rekening'); ?></th>
                            <th><?php echo Yii::t('trans', 'Jumlah (Kg)'); ?><br/>(a)</th>
                            ...
                            <tr class="template" style="display: none;">
                                <td>
                                    ...
                                    <select class="span3 required" name="items[x][kode_rekening_id]" id="items_xxkode_rekening_id" onchange="getNamaRekening($(this).val(), $(this).attr('id').split('x'));">
                                        ...
                                        foreach (KodeRekening::model()->getParentTreeOptions(Spt::PARENT_WALET) as $id => $val) {
                                            echo '<option value=' . $id . '>' . $val . '</option>';
                                        }
                                        ?>
                                    </select>
                                </td>
                                <td><input name="items[x][nilai]" id="items_xxnilai" class="span1 required" type="text" onkeyup="getValueWalet();" style="text-align: right;"></td>
                        ...
                        <?php else : ?>         
                            ...
                            foreach ($model->sptItems as $item):
                                ...
                                <tr class="new">
                                    ...
                                    <td><input value="<?php echo $item->nilai; ?>" name="items[<?php echo $index; ?>][nilai]" id="items_<?php echo $index; ?>xnilai" class="span1 required" type="text" onkeyup="getValueWalet();" style="text-align: right;"></td>
...
<script type="text/javascript">
    ...
    function getValueWalet() {
        clearInterval(timer);  //clear any interval on key up
        timer = setTimeout(function () { //then give it a second to see if the user is finished
            jQuery.ajax({'type': 'POST', 'url': '<?php echo $this->createUrl('spt/ajaxGetValueWalet'); ?>', 'cache': false, dataType: 'json', 'data': $('#spt-form').serialize()}).done(function (data) {
