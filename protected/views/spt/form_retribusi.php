...
$this->modulTitle = Yii::t('trans', 'Create') . ' ' . Yii::t('trans', 'SPTRD Retribusi');
$this->menu = array(
    array('label' => Yii::t('trans', 'Manage'), 'url' => array('index', 'jenis' => Spt::JENIS_PAJAK_RETRIBUSI), 'icon' => 'list-alt', 'visible' => (Yii::app()->util->is_authorized('spt.index')) ? true : false),
);
?>
<div class="form">
    ...
    <div class="row">
        <div class="span5">
            ...
            echo $form->select2ActiveTextFieldControlGroup($model, 'npwpd', array(
                'label' => Yii::t('trans', 'NPWRD'),
                'style' => 'width:300px',
                'onChange' => 'fillData($(this).val());',
                'options' => array(
                    'allowClear' => true,
                    'placeholder' => Yii::t('trans', 'Ketik nama atau npwrd'),
                    'ajax' => array(
                        'url' => Yii::app()->createUrl('wajibPajak/jsonNpwpd?jenis=r'),
                        'dataType' => 'json',
                        'quietMillis' => 100,
        ...
        <div class="span5">
            ...
            <?php // echo $form->dropdownListControlGroup($model, 'kode_rekening_id', KodeRekening::model()->getParentTreeOptions(Spt::PARENT_RETRIBUSI), array('span' => 3, 'empty' => Yii::t('trans', '-- Pilih Kode Rekening --'), 'onchange' => 'getNamaRekening($(this).val())')); ?>
        ...
        <div class="span-12">
            <fieldset><legend><?php echo Yii::t('trans', 'Detail Retribusi'); ?></legend>
                            ...
                            <tr class="template" style="display: none;">
                                <td>
                                    ...
                                    <select class="span5 required" name="items[x][kode_rekening_id]" id="items_xxkode_rekening_id" onchange="getNamaRekening($(this).val(), $(this).attr('id').split('x'));">
                                        ...
                                        foreach (KodeRekening::model()->getParentTreeOptions(Spt::PARENT_RETRIBUSI) as $id => $val) {
                                            echo '<option value=' . $id . '>' . $val . '</option>';
                                        }
                                        ?>
                                    </select>
                                </td>
                                <td><input name="items[x][nilai]" id="items_xxnilai" class="span3 required" type="text" onkeyup="getValueRetribusi();" style="text-align: right;"></td>
                        ...
                        <?php else : ?>         
                            ...
                            foreach ($model->sptItems as $item):
                                ...
                                <tr class="new">
                                    ...
                                    <td><input value="<?php echo $item->nilai; ?>" name="items[<?php echo $index; ?>][nilai]" id="items_<?php echo $index; ?>xnilai" class="span3 required" type="text" onkeyup="getValueRetribusi();" style="text-align: right;"></td>
                                    <td><input value="<?php echo $item->tarif_dasar; ?>" readonly="readonly" name="items[<?php echo $index; ?>][tarif_dasar]" id="items_<?php echo $index; ?>xtarif_dasar" class="span1" type="text" style="text-align: right;"></td>
...
<script type="text/javascript">
    ...
    function getValueRetribusi() {
        clearInterval(timer);  //clear any interval on key up
        timer = setTimeout(function () { //then give it a second to see if the user is finished
            jQuery.ajax({'type': 'POST', 'url': '<?php echo $this->createUrl('spt/ajaxGetValueRetribusi'); ?>', 'cache': false, dataType: 'json', 'data': $('#spt-form').serialize()}).done(function (data) {
