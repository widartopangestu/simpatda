...
$this->breadcrumbs = array(
    Yii::t('trans', 'SPTPD') => array('index', 'jenis' => Spt::JENIS_PAJAK_AIR),
    Yii::t('trans', 'Create') . ' ' . Yii::t('trans', 'SPTPD Pajak Air Bawah Tanah'),
);
$this->pageTitle = Yii::app()->params['title'] . ' - ' . Yii::t('trans', 'Create') . ' ' . Yii::t('trans', 'SPTPD Pajak Air Bawah Tanah');
$this->modulTitle = Yii::t('trans', 'Create') . ' ' . Yii::t('trans', 'SPTPD Pajak Air Bawah Tanah');
$this->menu = array(
    array('label' => Yii::t('trans', 'Manage'), 'url' => array('index', 'jenis' => Spt::JENIS_PAJAK_AIR), 'icon' => 'list-alt', 'visible' => (Yii::app()->util->is_authorized('spt.index')) ? true : false),
);
?>
<div class="form">
    ...
    <div class="row">
        ...
        <div class="span5">
            ...
            <?php // echo $form->dropdownListControlGroup($model, 'kode_rekening_id', KodeRekening::model()->getParentTreeOptions(Spt::PARENT_AIR), array('span' => 3, 'empty' => Yii::t('trans', '-- Pilih Kode Rekening --'), 'onchange' => 'getNamaRekening($(this).val())')); ?>
        ...
        <div class="span-12">
            <fieldset><legend><?php echo Yii::t('trans', 'Detail Air Tanah'); ?></legend>
                <table class="table table-striped table-bordered items-list">
                    <thead>
                        <tr>
                            <th><?php echo Yii::t('trans', 'Kode Rekening'); ?></th>
                            <th><?php echo Yii::t('trans', 'Lokasi'); ?><br/>(a)</th>
                            <th><?php echo Yii::t('trans', 'Jumlah (M<sup>3</sup>)'); ?><br/>(b)</th>
                            <th><?php echo Yii::t('trans', 'Tarip Dasar'); ?><br/>(c)</th>
                            <th><?php echo Yii::t('trans', 'Dasar Pengenaan (Rp.)'); ?><br/>(d = (b x c))</th>
                            <th><?php echo Yii::t('trans', 'Tarip (%)'); ?><br/>(e)</th>
                            <th><?php echo Yii::t('trans', 'Pajak (Rp.)'); ?><br/>(d x e)</th>
                            <th>#</th>
                        </tr>
                    </thead>
                    <tbody>
                            ...
                            <tr class="template" style="display: none;">
                                <td>
                                    ...
                                    <select class="span3 required" name="items[x][kode_rekening_id]" id="items_xxkode_rekening_id" onchange="getNamaRekening($(this).val(), $(this).attr('id').split('x'));">
                                        ...
                                        foreach (KodeRekening::model()->getParentTreeOptions(Spt::PARENT_AIR) as $id => $val) {
                                            echo '<option value=' . $id . '>' . $val . '</option>';
                                        }
                                        ?>
                                    </select>
                                </td>
                                <td><input name="items[x][lokasi]" id="items_xxlokasi" class="span2 required" type="text"></td>
                                <td><input name="items[x][nilai]" id="items_xxnilai" class="span1 required" type="text" onkeyup="getValueAir();" style="text-align: right;"></td>
                                <td><input readonly="readonly" name="items[x][tarif_dasar]" id="items_xxtarif_dasar" class="span1" type="text" style="text-align: right;"></td>
                        ...
                        <?php else : ?>         
                            ...
                            foreach ($model->sptItems as $item):
                                ...
                                <tr class="new">
                                    ...
                                    <td><input value="<?php echo $item->lokasi; ?>" name="items[<?php echo $index; ?>][lokasi]" id="items_<?php echo $index; ?>xnilai" class="span2 required"></td>
                                    <td><input value="<?php echo $item->nilai; ?>" name="items[<?php echo $index; ?>][nilai]" id="items_<?php echo $index; ?>xnilai" class="span1 required" type="text" onkeyup="getValueAir();" style="text-align: right;"></td>
                                    <td><input value="<?php echo number_format($item->tarif_dasar, Yii::app()->params['currency_precision']); ?>" readonly="readonly" name="items[<?php echo $index; ?>][tarif_dasar]" id="items_<?php echo $index; ?>xtarif_dasar" class="span1" type="text" style="text-align: right;"></td>
                        ...
                        <?php endif; ?>
                        <tr>
                            <td colspan="5"><a href="#" class="btn btn-primary add-items">
...
<script type="text/javascript">
    ...
    function getValueAir() {
        clearInterval(timer);  //clear any interval on key up
        timer = setTimeout(function () { //then give it a second to see if the user is finished
            jQuery.ajax({'type': 'POST', 'url': '<?php echo $this->createUrl('spt/ajaxGetValueAir'); ?>', 'cache': false, dataType: 'json', 'data': $('#spt-form').serialize()}).done(function (data) {
